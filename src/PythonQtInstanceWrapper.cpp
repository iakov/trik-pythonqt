...
int PythonQtInstanceWrapper_init(PythonQtInstanceWrapper * self, PyObject * args, PyObject * kwds)
{
  ...
  if (self->classInfo()->constructors()) {
	...
	if (directCPPPointer) {
	  ...
	  // if the constructor has a PythonQtPassThisOwnership parameter and that owner is not nullptr,
	  // this wrapper is immediately owned by CPP
	  // (Example: QGraphicsItem(QGraphicsItem* parent), if parent != nullptr, ownership needs to be passed
	  //  to C++ immediately)
...

	  {
		...
		if (member._type == PythonQtMemberInfo::Slot) {
		  ...
		  if (result) {
			...
			// in case of result == nullptr, expect that the code as thrown a std::exception
...
// we override nb_nonzero, so that one can do 'if' expressions to test for a nullptr ptr
