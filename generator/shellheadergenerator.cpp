...
void ShellHeaderGenerator::write(QTextStream &s, const AbstractMetaClass *meta_class)
{
  ...
  if (meta_class->generateShellClass() && !ctors.isEmpty()) {
    ...
    s << "class " << shellClassName(meta_class)
	  << " : public " << meta_class->qualifiedCppName() << "\n{\n";
	s << "public:\n";
    ...
    if (meta_class->isQObject()) {
	  s << "  const QMetaObject* metaObject() const;\n";
	  s << "  int qt_metacall(QMetaObject::Call call, int id, void** args);\n";
    }
	...
	s << "  PythonQtInstanceWrapper* _wrapper;\n";
    ...
    foreach(AbstractMetaFunction* fun, promoteFunctions) {
      ...
      if (fun->wasProtected()) {
        ...
        if (!fun->isStatic()) {
		  ...
		  s << "((" << fun->ownerClass()->qualifiedCppName() << "*)this)->";
        }
        s << fun->originalName() << "(";
    ...
    if (flags.count()) {
      foreach(AbstractMetaEnum* enum1, enums) {
        if (enum1->typeEntry()->flags()) {
		  ...
		  s << "Q_DECLARE_FLAGS("<< origName << ", " << enum1->name() <<")\n";
        }
      }
    }
  }
  ...
  if (meta_class->generateShellClass() || !meta_class->isAbstract()) {
    ...
    if (meta_class->typeEntry()->isValue()
	  ...
	  s << meta_class->qualifiedCppName() << "* new_" << meta_class->name() << "(const " << meta_class->qualifiedCppName() << "& other) {\n";
	  s << className << "* a = new " << className << "();\n";
	  s << "*((" << meta_class->qualifiedCppName() << "*)a) = other;\n";
	  s << "return a; }\n";
    }
  }
